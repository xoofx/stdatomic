...
*** Clang's =__c11_atomic= builtins

    Clang has gone a different path for the builtins that implement
...
*** The =__sync= ABI
    ...
    basically the same builtins for arithmetic types as above, only
    that
...
*** The lockfull fallback functions

    In absence of proper architecture support, the all fallbacks (for
    the three builtin families) with =_X= suffix use the ones without
   ...
   =memcopy= and =memcmp= for load, store and comparison.

   These functions *cannot be called directly* from within your code,
   because the compiler cannot distinguish them from the gcc builtins,
...
** Partial C11 atomics support
  ...
  The versions provide the builtin functions as described above but
...
** Basic atomics support
   ...
   Even older versions of gcc and clang implement the =__sync= builtin
   ...
   builtins only have fully synchronizing versions, they will not be
