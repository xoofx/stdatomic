...
set output 'optimized-comparison.eps'
unset clip points
set clip one
unset clip two
...
set title "musl, stress test, futex and pthread_mutex_t, forced descheduling with probability p"
...
paste(file, base) = sprintf("< paste %s-yield-%s.dat %s.dat", base, file, base)
files = "32 2"
pthread = "optimized-pthread"
futex = "optimized-lockfull"
plot                                                                                             \
     for [file in files] paste(file, futex) using 1:($2) with lines title sprintf("futex, p = 1/%s", file),\
     for [file in files] paste(file, pthread) using 1:($2) with lines title sprintf("pthread, p = 1/%s", file)
