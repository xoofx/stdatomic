...
** Atomic instructions on modern hardware
   ...
   power of 2, they must rely on external or internal /locks/ that
   protect a /critical section/, CS. That is they need an auxiliary
   object that protects the data object by means of some /lock
   primitives/ and by memorizing a /state/ of the application.
...
#+END_SRC
  ...
  - A first spinning phase attempts the atomic operation several
    ...
    avoid sending threads into sleep.
...
#+END_SRC
   ...
   2. Otherwise, we enter an acquisition loop.

      1. First, we spin for a while (determined below) to set the HO
         bit as well, and thus acquire the lock.

      2. If that times out, we suppose that the lock is
         under congestion and we go into a =futex_wait=.
...
** Analysis
   ...
   operation for fast =lock= and =unlock=. It remains to show that it
   cannot deadlock.
...
** The framework
   ...
   pointers for the head to avoid the ABA problem, \cite{IBM370,michael04:aba}.
...
** The test program
   ...
   - a spin lock using =atomic_compare_exchange_strong=
   - the compilers "native" generic lock, also a mutex
...
*** An =arm7= machine with 4 cores
    ...
    instructions for word sizes up to 64 bit.
...
*** A =x86_64= machine with 2x2 hypethreaded cores
    ...
    has atomic instructions for word sizes up to 128 bit.
